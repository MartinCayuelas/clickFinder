...
object Predictor {
  ...
  def predict(spark: SparkSession, dataPath: String): Unit = {
    ...
    val dataCleaned = DataCleaner.clean(dataSelected)
    ...
    val dataAssembled = assembler.transform(dataCleaned).select($"features", $"label")


    val model = LogisticRegressionModel.load("models/balancedModel").setPredictionCol("label").setFeaturesCol("features")
    val predictions = model
      .transform(dataAssembled.select("features"))
